...
import {
  createGmicloudV4Middleware,
  GmicloudV4UsageAccounting,
} from './language-model-v4-gmicloud-billing-middleware.js';
import {
  BillingEventSchema,
  MockLanguageModelV4,
...
describe('GmicloudBillingMiddlewareV4 Integration', () => {
  ...
  const createResult = (
    ...
    content: [
      ...
      {
        type: 'reasoning',
        text: 'We need answer simple. Capital of Sweden is Stockholm.',
      },
    ],
    warnings: [],
    ...
    usage: {
      ...
      // `POST /api/raw/gmicloud/generate-text` route (via `@ai-sdk/gmicloud`'s `createGmicloud()`)
      // for the `deepseek-ai/DeepSeek-V4-Flash-0731` model asked "What is the capital of Sweden?".
      raw: {
        prompt_tokens: 90,
      ...
      } as GmicloudV4UsageAccounting,
    },
    response: { id: 'resp_gmicloud_abc123', timestamp: new Date() },
    providerMetadata: {},
    ...overrides,
  });
  ...
  describe('wrapGenerate', () => {
    it('should extract usage, resolve pricing, calculate cost, and broadcast event using the sample generate-text usage', async () => {
      ...
      const middleware = createGmicloudV4Middleware({
      ...
      const rawUsage = baseResult.usage.raw as GmicloudV4UsageAccounting;
    ...
    it('should deduct reasoning tokens from the completion bucket and report usage.reasoningTokens for the captured DeepSeek-V4-Flash-0731 sample', async () => {
      const destinationSpy = vi.fn();
      const middleware = createGmicloudV4Middleware({
        destinations: [destinationSpy],
        priceResolver: mockPriceResolver,
      ...
      expect(parsedEvent.usage.inputTokens).toBe(90);
      expect(parsedEvent.usage.outputTokens).toBe(14);
      expect(parsedEvent.usage.reasoningTokens).toBe(11);

      // prompt: 0.000001 * 1e9 * (90 - 0) = 90,000 nanos
      // completion: 0.000003 * 1e9 * (14 - 11) = 9,000 nanos
      // reasoning: 0.000003 * 1e9 * 11 = 33,000 nanos
      // Total: 90,000 + 9,000 + 33,000 = 132,000 nanos
      expect(parsedEvent.cost?.amount).toBe(132000);
