...
export async function fetchPolarUsage(userId: string): Promise<PolarUsageData> {
  const empty = { consumedUnits: 0, creditedUnits: 0, meterName: 'Usage', found: false };
  ...
  try {
    const page = await polar.customerMeters.list({ externalCustomerId: userId, meterId: config.meterId, limit: 1 });
