...
/**
 * Token usage passed into {@link calculateGoogleCost}.
 *
 * Counts match a Google-style completion: prompt vs completion, cache read/write, and reasoning. The Google
 * billing middleware maps AI SDK `usage` into this shape.
 */
export interface GoogleCostInputs {
...
/**
 * Computes total cost for a Google completion from {@link ModelPricing} and token usage.
 *
 ...
 * in your table; `usage` is token counts as {@link GoogleCostInputs}.
 * @returns A {@link Cost}, or `undefined` when `pricing` is missing.
 * @internal
 */
export const calculateGoogleCost = (params: {
  pricing: ModelPricing | undefined;
  usage: GoogleCostInputs;
}): Cost | undefined => {
