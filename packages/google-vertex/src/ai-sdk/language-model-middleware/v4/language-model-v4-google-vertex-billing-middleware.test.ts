...
import { createGoogleVertexV4Middleware } from './language-model-v4-google-vertex-billing-middleware.js';
import {
  BillingEventSchema,
  MockLanguageModelV4,
...
describe('GoogleVertexBillingMiddlewareV4 Integration', () => {
  ...
  describe('wrapGenerate', () => {
    it('should extract usage, resolve pricing, calculate cost, and broadcast event using the captured gemini-2.5-flash payload', async () => {
      ...
      const middleware = createGoogleVertexV4Middleware({
    ...
    it('should compute identical cost for a synthetic PROVISIONED-traffic call as an equivalent ON_DEMAND call (known, tested billing gap)', async () => {
      ...
      const onDemandMiddleware = createGoogleVertexV4Middleware({
        destinations: [onDemandDestinationSpy],
        priceResolver: mockPriceResolver,
      });

      const provisionedDestinationSpy = vi.fn();
      const provisionedMiddleware = createGoogleVertexV4Middleware({
      ...
      const onDemandModel = new MockLanguageModelV4({
        modelId: 'gemini-2.5-flash',
        provider: 'google.vertex.chat',
        doGenerate: async () => createResult(),
      });
      const provisionedModel = new MockLanguageModelV4({
        modelId: 'gemini-2.5-flash',
        provider: 'google.vertex.chat',
