 ...
 */
export interface AzureV4UsageAccounting extends JSONObject {
  input_tokens?: number;
...
/**
 * Configuration for {@link createAzureV4Middleware}.
 *
 ...
 */
export interface AzureV4MiddlewareOptions<
 ...
 *
 * @typeParam TTags - The shape of the tags object, extending {@link DefaultTags}.
 * @param options - Billing options; see {@link AzureV4MiddlewareOptions}. A `priceResolver` is required.
 * @returns A V4 billing middleware instance for Azure.
 *
 * @example
 * ```ts
 ...
 * ```
 */
export function createAzureV4Middleware<TTags extends DefaultTags>(
  options: AzureV4MiddlewareOptions<TTags>,
) {
  return createV4BillingMiddleware<TTags>({
    ...
    buildEvent: async ({
      ...
      const azureRawUsage = usage?.raw as AzureV4UsageAccounting | undefined;
