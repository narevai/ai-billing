...
import { calculateDeepinfraCost } from './calculate-deepinfra-cost.js';
...
describe('calculateDeepinfraCost', () => {
  it('should return undefined if no pricing is provided', () => {
    const result = calculateDeepinfraCost({
      pricing: undefined,
      usage: {
        promptTokens: 10,
        completionTokens: 10,
  ...
  it('should calculate basic cost correctly', () => {
    ...
    const result = calculateDeepinfraCost({ pricing: mockPricing, usage });
  ...
  it('should calculate cost for the sample DeepInfra generate-text usage (prompt_tokens: 17, completion_tokens: 8, cached_tokens: 0)', () => {
    const mockPricing: ModelPricing = {
  ...
  it('should deduct reasoning tokens from completion tokens correctly (speculative DeepSeek-R1-style usage)', () => {
    const mockPricing: ModelPricing = {
  ...
  it('should default cacheReadTokens to 0 when omitted from usage (defensive fallback for non-strict callers)', () => {
    ...
    const result = calculateDeepinfraCost({ pricing: mockPricing, usage });
    const resultWithExplicitZero = calculateDeepinfraCost({
      pricing: mockPricing,
