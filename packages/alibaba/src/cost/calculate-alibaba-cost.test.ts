...
import { calculateAlibabaCost } from './calculate-alibaba-cost.js';
import type { ModelPricing, CostInputs } from '@ai-billing/types';

describe('calculateAlibabaCost', () => {
  it('should return undefined if no pricing is provided', () => {
    const result = calculateAlibabaCost({
      pricing: undefined,
      usage: {
        promptTokens: 10,
        completionTokens: 10,
  ...
  it('should calculate basic cost correctly', () => {
    ...
    const result = calculateAlibabaCost({ pricing: mockPricing, usage });
  ...
  it('should calculate cost for the captured qwen-plus generate-text usage (prompt_tokens: 15, completion_tokens: 10, cached_tokens: 0)', () => {
    const mockPricing: ModelPricing = {
    ...
    // Prompt: 0.0000002 * 1e9 * 15 = 3,000 nanos
  ...
  it('should deduct reasoning tokens from completion tokens correctly (Qwen thinking-mode usage)', () => {
    const mockPricing: ModelPricing = {
  ...
  it('should default cacheReadTokens to 0 when omitted from usage (defensive fallback for non-strict callers)', () => {
    ...
    const result = calculateAlibabaCost({ pricing: mockPricing, usage });
    const resultWithExplicitZero = calculateAlibabaCost({
      pricing: mockPricing,
  ...
  it('should default reasoningTokens to 0 when omitted from usage (defensive fallback for non-strict callers)', () => {
    ...
    // Total: 5,400 + 300 + 144,500 = 150,200 nanos
    expect(result).toEqual({
      amount: 150200,
      unit: 'nanos',
      currency: 'USD',
