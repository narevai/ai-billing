[**@ai-billing/openai-compatible**](../index)

***

[@ai-billing/openai-compatible](/sdk/ai-billing/reference/openai-compatible/typedoc/index) / OpenAICompatibleV4MiddlewareOptions

# Interface: OpenAICompatibleV4MiddlewareOptions\<TTags\>

Defined in: [openai-compatible/src/ai-sdk/language-model-middleware/v4/language-model-v4-openai-compatible-billing-middleware.ts:27](https://github.com/narevai/ai-billing/blob/main/packages/openai-compatible/src/ai-sdk/language-model-middleware/v4/language-model-v4-openai-compatible-billing-middleware.ts#L27)

Configuration for [createOpenAICompatibleV4Middleware](/sdk/ai-billing/reference/openai-compatible/typedoc/functions/createOpenAICompatibleV4Middleware).

Extends BaseBillingMiddlewareOptions (`destinations`, `defaultTags`, `waitUntil`, `onError`) and
adds `priceResolver` plus `providerId`. Usage comes from the AI SDK's normalized usage fields for
OpenAI-compatible responses; cost is computed from that usage and the resolved ModelPricing using
the same rules as the package's cost helper.

`providerId` must match the `name` you pass to `createOpenAICompatible` from `@ai-sdk/openai-compatible`
so PriceResolver calls and emitted `provider` on events stay aligned.

## Extends

- `BaseBillingMiddlewareOptions`\<`TTags`\>

## Type Parameters

### TTags

`TTags` *extends* `DefaultTags`

The shape of the tags object, extending DefaultTags.

## Properties

### priceResolver

> **priceResolver**: `PriceResolver`

Defined in: [openai-compatible/src/ai-sdk/language-model-middleware/v4/language-model-v4-openai-compatible-billing-middleware.ts:30](https://github.com/narevai/ai-billing/blob/main/packages/openai-compatible/src/ai-sdk/language-model-middleware/v4/language-model-v4-openai-compatible-billing-middleware.ts#L30)

***

### providerId

> **providerId**: `string`

Defined in: [openai-compatible/src/ai-sdk/language-model-middleware/v4/language-model-v4-openai-compatible-billing-middleware.ts:31](https://github.com/narevai/ai-billing/blob/main/packages/openai-compatible/src/ai-sdk/language-model-middleware/v4/language-model-v4-openai-compatible-billing-middleware.ts#L31)

***

### destinations?

> `optional` **destinations?**: `Destination`\<`TTags`\>[]

Defined in: types/dist/index.d.ts:71

One or more billing destinations that receive each emitted BillingEvent.

#### Inherited from

`BaseBillingMiddlewareOptions.destinations`

***

### defaultTags?

> `optional` **defaultTags?**: `TTags`

Defined in: types/dist/index.d.ts:73

Tags merged into every emitted event.

#### Inherited from

`BaseBillingMiddlewareOptions.defaultTags`

***

### waitUntil?

> `optional` **waitUntil?**: (`promise`) => `void`

Defined in: types/dist/index.d.ts:78

Edge-runtime hook (e.g. `ctx.waitUntil`) used to keep the process alive
while billing events are flushed asynchronously.

#### Parameters

##### promise

`Promise`\<`unknown`\>

#### Returns

`void`

#### Inherited from

`BaseBillingMiddlewareOptions.waitUntil`

***

### onError?

> `optional` **onError?**: (`error`) => `void`

Defined in: types/dist/index.d.ts:80

Called when an error occurs during event extraction or dispatch. Defaults to a silent no-op.

#### Parameters

##### error

`unknown`

#### Returns

`void`

#### Inherited from

`BaseBillingMiddlewareOptions.onError`
